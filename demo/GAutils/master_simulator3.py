...
def main():
    ...
    glena = np.zeros((cfg.Ninst, 2*max(Noba))) # Keep enough space for 2x False alarms
...
#        plt.plot(rng_used, 10*np.log10(np.mean(np.sqrt(crb1[:,:,:,i]), axis=(1,2))), 'k--', label='CRB')
    ...
    handle.write('SAGA: hN={}, hscale={}, incr ={}\n'.format(cfg.hN, cfg.hscale, cfg.incr))
    handle.write('Misc: rd_wt={}, fu_alg={}, gn_steps={}\n'.format(cfg.rd_wt, cfg.fu_alg, cfg.gn_steps))
    handle.write('d_ospa = {}m, c_ospa = {}'.format(10*np.sqrt(min([np.sum(sensor.getnominalCRB()[0]) for sensor in sensors])), 10*np.sqrt(min([np.sum(sensor.getnominalCRB()) for sensor in sensors]))))
