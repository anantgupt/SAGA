...
#            print('T=({},{}),E=({},{})'.format(true_ob.x,true_ob.y,est_ob.x,est_ob.y))
    ...
    err_pos, err_vel =0,0
    c = 5*np.sqrt(min([np.sum(sensor.getnominalCRB()) for sensor in sensors])) # previous was 5
    cpos = 5*np.sqrt(min([np.sum(sensor.getnominalCRB()[0]) for sensor in sensors])) # was 10
    c_pen = c # Penalty for miss: 1
