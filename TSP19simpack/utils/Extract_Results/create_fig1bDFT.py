...
def cf1bdft(mode = 'Relax', width = 3.45, height = 2.6, font_size = 8):
    #fig_handle = pl.load(open('results6_14/fig_obj_est2/plot4.pickle','rb'))
    fig_handle1 = pl.load(open('fig_snr-Nob10/plot4.pickle','rb'))
    fig_handle2 = pl.load(open('fig_snr-Nob20/plot4.pickle','rb'))
    fig_handle3 = pl.load(open('fig_DFT_snr-Nob10/plot4.pickle','rb'))
    fig_handle4 = pl.load(open('fig_DFT_snr-Nob20/plot4.pickle','rb'))
    ...
    for i in range(2):
        ...
        ax[i].plot(rng, crb3, 'b--')
        ax[i].plot(rng, crb4, 'g--')       
        ax[i].plot(rng, mse1, 'b-', marker =',', label='NOMP Nob=10')
        ax[i].plot(rng, mse2, 'g-',marker ='.', label='NOMP Nob=20')
        ax[i].plot(rng, mse3, 'b-', marker ='+',label='DFT Nob=10')
        ax[i].plot(rng, mse4, 'g-', marker ='x',label='DFT Nob=20')
        ax[i].legend(loc='lower left'),ax[i].grid(True);ax[i].set_xlabel('SNR (dB)');
    ax[0].set_title('Range Error');ax[0].set_ylabel('RMSE (dB) (m)')
    ax[1].set_title('Doppler Error');ax[1].set_ylabel('RMSE (dB) (m/s)')
    ...
    pl.dump(fig, open("Sel_figs/DFTplot_RD_error.pickle", "wb"))
    fig.savefig('Sel_figs/DFTplot_RD_error.pdf')
