...
def cf5adft(mode = 'Relax', width = 3.45, height = 2.6, font_size = 8):
    #fig_handle = pl.load(open('results6_14/fig_obj_est2/plot4.pickle','rb'))
    fig_handle1 = pl.load(open('fig_Nsens-Nob10/plot3.pickle','rb'))
    fig_handle2 = pl.load(open('fig_Nsens-Nob20/plot3.pickle','rb'))

    fig_handle1d = pl.load(open('fig_DFT_Nsens-Nob10/plot3.pickle','rb'))
    fig_handle2d = pl.load(open('fig_DFT_Nsens-Nob20/plot3.pickle','rb'))
    ...
    for i in range(2):
        ...
        mse1 = fig_handle1.axes[i*2].lines[0].get_data()[1]
        mse2 = fig_handle2.axes[i*2].lines[0].get_data()[1]
        mse1d = fig_handle1d.axes[i*2].lines[0].get_data()[1]
        mse2d = fig_handle2d.axes[i*2].lines[0].get_data()[1]
        
        ax[i].plot(rng, mse1, 'g-', label=mode+r',NOMP $N_t=10$')
        ax[i].plot(rng, mse2, 'r-', label=mode+r',NOMP $N_t=20$')
        ax[i].plot(rng, mse1d, 'b--', label=mode+r',DFT $N_t=10$')
        ax[i].plot(rng, mse2d, 'y--', label=mode+r',DFT $N_t=20$')
    ...
    pl.dump(fig, open("Sel_figs/DFT_plot_OSPA_vs_Nsens-Nob.pickle", "wb"))
    fig.savefig('Sel_figs/DFT_plot_OSPA_vs_Nsens-Nob.pdf')