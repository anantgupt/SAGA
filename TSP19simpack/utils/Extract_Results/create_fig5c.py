...
def cf5c(mode = 'Relax', width = 3.45, height = 2.6, font_size = 8):
    #fig_handle = pl.load(open('results6_14/fig_obj_est2/plot4.pickle','rb'))
    fig_handle1 = pl.load(open('fig_Nob-rob20/plot3.pickle','rb'))
    fig_handle2 = pl.load(open('fig_Nob-rob0/plot3.pickle','rb'))
    fig_handle3 = pl.load(open('fig_Nob-rob1/plot3.pickle','rb'))
    fig_handle4 = pl.load(open('fig_Nob-rob2/plot3.pickle','rb'))
    ...
    for i in range(2):
        rng = fig_handle2.axes[i*2].lines[0].get_data()[0]
    ...
    #    crb3 = fig_handle3.axes[i].lines[1].get_data()[1]
        ...
        ax[i].plot(rng, mse2[i], 'g-', label=mode+r', $\rho=0$')
    
        ax[i].plot(rng, mse3[i], 'g-.', label=mode+r', $\rho=1$')

        ax[i].plot(rng, mse4[i], 'g-x', label=mode+r', $\rho=2$')
    
        ax[i].plot(rng, mse1[i], 'g-v', label=mode+r', $\rho=\infty$')
    ...
    pl.dump(fig, open("Sel_figs/plot_OSPA_vs_Nob-rob.pickle", "wb"))
    fig.savefig('Sel_figs/plot_OSPA_vs_Nob-rob.pdf')