...
def cf5a(mode = 'Relax', width = 3.45, height = 2.6, font_size = 8):
    ...
    for i in range(2):
        ...
        ax[i].plot(rng, mse1[i], 'g-', label=mode+r', $N_t=10$')
    
        ax[i].plot(rng, mse2[i], 'g-.', label=mode+r', $N_t=20$')
    ...
    pl.dump(fig, open("Sel_figs/plot_OSPA_vs_Nsens-Nob.pickle", "wb"))
    fig.savefig('Sel_figs/plot_OSPA_vs_Nsens-Nob.pdf')