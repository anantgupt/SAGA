...
# Load figure 11 from disk and display
if True:
    width = 3.45
    ...
    si=['0','1','2','4','7']
    for i in range(5):
        fha[i][0] = pl.load(open('SAGA/fig_Nsens-rob'+si[i]+'/plot3.pickle','rb'))
        fha[i][1] = pl.load(open('SAGA/fig_Nsens-rob'+si[i]+'/plot1.pickle','rb')) # Use plot11 for ops
        fha[i][2] = pl.load(open('SAESL/fig_Nsens-rob'+si[i]+'/plot3.pickle','rb'))
        fha[i][3] = pl.load(open('SAESL/fig_Nsens-rob'+si[i]+'/plot1.pickle','rb')) # Use plot11 for ops
...
#    ax.yaxis.set_minor_formatter(None)
    ...
    # Load figure 12 from disk and display
if True:
    width = 3.45
    golden_mean = (np.sqrt(5)-1.0)/2.0
    height = width*golden_mean
    ...
    for i in range(4):
        fha[i][0] = pl.load(open('SAGA/fig_Nsens-rob'+si[i]+'/plot3.pickle','rb'))
        fha[i][1] = pl.load(open('SAGA/fig_Nsens-rob'+si[i]+'/plot11.pickle','rb'))
        fha[i][2] = pl.load(open('SAESL/fig_Nsens-rob'+si[i]+'/plot11.pickle','rb'))
    ...
    if 0:
        ...
        for i, sis in enumerate(si):
            ...
            ax2.plot(rng, fha[i][2].axes[1].lines[2].get_data()[1]+fha[i][2].axes[1].lines[3].get_data()[1],'m-.')    # Rntime of SAESL
