...
class SignatureTracks: # collection of associated ranges[], doppler[] & estimated vx(scalar).
    ...
    def get_newfit_error_grp(cls, sensors, tnd_grp, sidnew, upper_thres):
        # Reports geometry fitting error for given R,D pair
        Ngrp = len(tnd_grp)
        rn = numpy.matlib.repmat(cls.r, Ngrp,1)
        dn = numpy.matlib.repmat(cls.d, Ngrp,1)
        rn = np.block([rn, np.array([[tnd.r for tnd in tnd_grp]]).T])
        dn = np.block([dn, np.array([[tnd.d for tnd in tnd_grp]]).T])
        sindx_new = np.hstack((cls.sindx,sidnew))
        keyval = tuple(sindx_new)
        L = np.array([sensors[si].x for si in sindx_new])
        H = np.array([[sensors[si].x, 1] for si in sindx_new])
        ...
        if keyval in cls.CRBdict:
            ...
            cls.CRBdict[keyval] = CRB
        # Main estimator 
        x2, FAA, rank, s = np.linalg.lstsq(H,np.vstack((Me, Me2_centered)).T,rcond=None)
        # scaling factors 
        gc = []
        states = []
        valid_state_ids = []
        for tid in range(Ngrp):
            M1var = (np.sum( CRB * np.array([dn[tid]**2, rn[tid]**2]).T,1) + np.prod(CRB,1) )
            M2var = (4*CRB[:,0] * np.array( rn[tid]**2) + CRB[:,0]**2)# Ignoring higher order terms
            gc_val =  np.sum(cfg.rd_wt[0]*(Me[tid]-H@x2.T[tid])**2/M1var + cfg.rd_wt[1]*(Me2_centered[tid] -H@x2.T[Ngrp+tid])/M2var)
